...
(coherence $\PWintCoh$ vs $\PWexpCoh$; consistency $\PWintCon$ vs $\PWexpCon$; fluency $\PWintFlu$ vs $\PWexpFlu$),
so the open-model result is \emph{not} an artifact of integer scoring; the comparison to the reported benchmark is
fair on this axis. But on \textbf{fluency} every open model falls well
