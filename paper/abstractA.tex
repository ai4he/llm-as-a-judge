...
($\mathrm{SD}_{\text{logit}}=\NLIitemSDlogit$ vs model $\NLImodelSDlogit$), and model scale contributes only modestly.
A freshly authored, unseen NLI holdout (solved at $\FNbest$ by the large models) confirms this is genuine competence,
not memorization of these older corpora. We turn
this into a decision framework and an eight-point protocol for designing LLM-as-a-judge experiments. Extending the
evaluation to subjective, interpretive constructs, where the human ceiling is contested, is important future work.
