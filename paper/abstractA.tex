...
analysis. The organizing finding is that judge reliability is governed by \emph{item-level human disagreement}:
panel accuracy on low-disagreement items ($\NLIlowH$) far exceeds that on high-disagreement items ($\NLIhighH$),
item-level variance dominates dataset- and model-level variance, and model scale contributes only modestly. We turn
