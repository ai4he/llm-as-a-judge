...
Cross-domain evidence shows LLM-judge reliability is broadly domain-general; HCC needs a method-specific account. Human-centered research is built on methods
...
\subsection{Method}
Following the shared protocol, we ran a review-specific identification arm: 25 HCC/HCI/qualitative concept-block
...
dataset-level model. Contamination is unlikely to explain the shortfall: the recent (2024) MultiPICo corpus, which the
...
The gap pattern is consistent and statistically reliable per dataset (Table~\ref{tab:gapsoc}): on all four
datasets the judge-vs-ceiling gap is significantly more negative on low-disagreement items than on high-disagreement
items (non-overlapping 95\% CIs for GoEmotions and HateXplain), confirming that the shortfall is concentrated where
humans agree, not where the gold is noisy.

\begin{table}[t]\centering\small
\caption{Study 2. Judge-vs-ceiling gap (panel accuracy minus human ceiling) by item human-disagreement, per dataset,
with annotator/item-bootstrap 95\% CIs. The gap is largest (most negative) where humans most agree.}
\label{tab:gapsoc}
\input{tables/tab_gap_social}
\end{table}
...
% =====================================================================
\textbf{Ethics.} The review analyzes published research, not human subjects, and Study 2 reuses already-public,
