...
inter-rater agreement, which we flag as a limitation.
...
is the aspect on which the \emph{reported proprietary judge also scores lowest}. We are careful not to over-claim a
hard ``structural ceiling'': a noiseless judge returning the rounded human means would still attain
$\rho{\approx}\SEfluSelf$ on fluency, so the open models' especially low fluency correlation is part compression of
the reference and part a genuine residual gap, and probability-weighted scoring (which we did not run) could narrow
it. The defensible claim is the weaker one: gains from scale concentrate on the verifiable aspects, while the
hardest-to-rank aspect stays hard for every judge.
...
(gemma MEDIUM throughout). A clean control is sobering and we report it plainly: on freshly authored post-cutoff
text that no model can have seen, the high-recall model still scores \CCmaxOverlap{} overlap (\CCflag), i.e.\ it
confabulates plausible continuations regardless of memorization (Appendix~\ref{app:robust}). The uniform MEDIUM tiers
are therefore largely a \emph{probe artifact} for that model, not evidence of memorization, and we do \emph{not}
treat the verbatim probe as a reliable leakage test. The one interpretable signal is that the low-recall model scores
LOW on the recent MultiPICo, consistent with no memorization there; combined with the fact that MultiPICo is matched
only at its low human ceiling, this argues the social-computing shortfall reflects human disagreement rather than
pretraining exposure. We otherwise lean on the disagreement analysis, not the contamination probe, for our claims.
...
When we add those competitors and use item-cluster-robust standard errors, the apparent subjectivity effect is
\textbf{no longer significant} ($\mathrm{OR}=\GLMsubjOR$, $p=\GLMsubjP$); the robust item-level predictors are
base-rate skew ($\mathrm{OR}=\GLMbaseOR$, $p{<}.001$), model scale ($\mathrm{OR}=\GLMsizeOR$, $p{<}.001$), and
granularity ($\mathrm{OR}=\GLMgranOR$, $p=\GLMgranP$). A dataset-level meta-regression (the honest unit,
$n{=}\metaN$) gives a negative accuracy-subjectivity slope ($\metaSlope$) that is \emph{not} robust to
leave-one-dataset-out ($[\metaLODOmin,\metaLODOmax]$). We therefore do \emph{not} claim a clean dataset-level
``subjectivity'' coefficient, and we have tempered the earlier draft accordingly.
...
finding.
\textbf{Illusory validation (data-grounded, with a caveat about our own tool).} Agreement on older public benchmarks
can reflect memorization rather than competence \citep{bender2021parrots}, so ``validated'' verdicts need a
contamination check, but our clean control (Appendix~\ref{app:robust}) shows such checks are themselves hard: a naive
verbatim-recall probe false-positives on high-recall models even for text they cannot have seen. The lesson is
double, that contamination is a real threat to ``validated'' claims and that the community needs better contamination
diagnostics than surface-recall probes.
...
\section{Limitations}
% =====================================================================
Both reviews used a single-reviewer, LLM-assisted pipeline (logs released) rather than dual human reviewers; we
validated the rubric with an independent re-coding (Krippendorff $\alpha=\IRRsoneA$; Appendix~\ref{app:irr}) but the
re-coder is also LLM-assisted, so genuine human IRR remains future work, and the stance code is only moderately
reliable. Metrics are heterogeneous and synthesized within families rather than pooled; several Study-1a
domains/methods rest on small $n$ (e.g.\ $n{\le}3$), whose point estimates we treat as indicative and report only via
family-level aggregates. The empirical studies are bounded: Study 1b uses one promising-domain benchmark (SummEval)
and integer-only scoring; Study 3 covers subjective \emph{classification} on \STHRdatasets{} datasets with open
models from a single provider and one prompt family (no proprietary frontier judges; prompt-robustness and a
contamination clean control are reported in Appendix~\ref{app:robust}, but the contamination tiers remain a
calibrated caution). Interpretive qualitative coding, participant simulation, and design judgment are reviewed
(Study 2), not tested in Study 3. Findings reflect models available through mid-2026. Full protocol, rubric anchors,
