...
ordered augment $>$ replace $>$ simulate (an independent re-coding reproduces the verdict at $\alpha=\IRRstwoV$ and the
role at $\alpha=\IRRstwoRole$, though it is LLM-assisted and the interpretive stance code is only moderately
reproducible, $\alpha=\IRRstwoStance$). Simulation and design verdicts are review-derived, not tested here; the
empirical evaluation covers English affect, toxicity, and irony annotation. \textbf{Study 2} tests the field's central boundary empirically: we run a
...
human ceiling on the multi-class subjective constructs (and where accuracy nominally reaches it, on binary irony, only
by predicting the base rate, macro-F1 ${\sim}0.45$); tellingly, the judge-vs-ceiling gap is \emph{largest where humans
