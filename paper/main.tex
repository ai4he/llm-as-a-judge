...
groups, caricature, hallucinated usability issues). \textbf{Study 3} closes the
...
annotation (emotion, hate speech), with binary offensiveness the strongest case. Together the studies yield a
...
\emph{formal} LLM-as-a-judge evaluation---a panel of models, bias controls, agreement benchmarked against the human
ceiling, and contamination checked. Study 3 closes this gap empirically with a released, reproducible harness.

\subsection{Method}
We evaluate a panel of \STHRmodels{} open models served via \textbf{Clemson RCD's vLLM API} (OpenAI-compatible),
...
panel \emph{matches or exceeds} them---ChaosNLI-SNLI: 5/6 models \textbf{Validated}, jury 0.858 ($>$ ceiling 0.752);
ChaosNLI-MNLI \textbf{Promising}. Where humans agree strongly on a subjective social construct, the panel falls
...
the ceiling. Four further effects: (i) \textbf{granularity}---binary offensiveness (Promising) clearly beats 3-way
...
(iii) the \textbf{alt-test} is passed (winning rate $\ge$0.5) \emph{only} on NLI---on every subjective
social-computing task no model statistically earns the right to replace a human annotator; (iv) \textbf{juries}
raise accuracy but do not lift any subjective task above its human ceiling. The \textbf{contamination probe}
(Table~\ref{tab:study3}, last column) shows the strongest judges have non-trivial verbatim recall of the oldest sets
(e.g., MEDIUM on SNLI), caveating the NLI ``Validated'' result; the social-computing shortfall holds regardless of
tier.
...
\begin{table}[t]\centering\small
\caption{Study 3 results across \STHRdatasets{} datasets $\times$ \STHRmodels{} models (15{,}000 judgments).
...
constructs. \emph{Limitation:} the present datasets are predominantly higher-contamination; a dedicated
low-contamination clean condition (a post-cutoff annotated set) and additional qualitative-coding corpora are the
immediate next additions, alongside a mixed-effects model of agreement on family, size, subjectivity, and
contamination tier with FDR control.
